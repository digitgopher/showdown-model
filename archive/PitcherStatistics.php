...
class PitcherStatistics
{
    ...
    function __construct($rawData, $totDistribution, $chart_oob_map){
        // Set real data
        $this->real = array(
            'nameFirst' => '', 
            'nameLast' => '',
        ...
        // Get control and number of outs from mapping. Assume mapping is ordered lowest OOB to highest OOB.
        $oob = round($this->real['OOB'] * 10000);
        foreach ($chart_oob_map as $key => $value) {
            if($oob < $value[0]){
                $justLessThan = $key;
                break;
            }
        }
        if($justLessThan == null){
            echo 'OOB map not valid or something.';exit;
        }
        else{
            // Find which of the 2 values it is closer to
            $splitDifference = ($chart_oob_map[$justLessThan][0] - $chart_oob_map[$justLessThan - 1][0])/2;
        }
        // Set control and num outs
        echo '  ',$this->control = ($oob > $chart_oob_map[$justLessThan - 1][0] + $splitDifference) ? $chart_oob_map[$justLessThan][1][0] : $chart_oob_map[$justLessThan - 1][1][0];
        echo ','.$this->num_outs = ($oob > $chart_oob_map[$justLessThan - 1][0] + $splitDifference) ? $chart_oob_map[$justLessThan][1][1] : $chart_oob_map[$justLessThan - 1][1][1];
    }
...
//    }
    public function getFinalChartFormatted($chartMaxValue){
        // Convert the 0-1 distribution to 0 - chartMaxValue
        $val = 0;
        $convDists = array();
        foreach ($this->dists as $key => $value) {
            $value_normalized = $value*$chartMaxValue;
        ...
        // Adjust the distribution to fall in line with 
        
        // Convert to pretty integers for output
        $formatedChart = array();
        $old_val = "0";
...
//                }
    ...
    public function calculateAnalogs(){
        $this->analogs['PU'] = ($this->battersFaced() - $this->hits() - $this->totals['SO'] - $this->totals['BB']) * $this->totals['HR'] / $this->hits() / 100 / 18 / 16;
